...
- **楼中楼**：默认自动提取主接口自带 1~3 条热评预览；`--replies` 开启时多线程并发深挖（单楼 ≤20 条全爬 / ≤250 条取 40 / 250+ 取 60）
- **去噪规则**：过滤过短与纯符号水评；同一文本出现超过 3 次仅跳过当前条（不中断后续正常评论抓取）
- **防死循环**：基于评论唯一 ID（RPID）集合与游标状态判断，防止接口卡死
- `--limit N`：楼层目标上限，默认 200；设为 `0` 或 `--all-comments` 表示全量爬取（带时间预估、二次确认与 `-y` 选项）
